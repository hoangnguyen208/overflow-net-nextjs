...
const isTargetAnswer = (target: Question | Answer): target is Answer => 'questionId' in target;
...
export default function VotingButtons({target, currentUserId, askerId}: Props) {
    ...
    const handleAddVote = (value: 1 | -1) => () => {
        setTargetBtn({type: value === 1 ? 'up' : 'down', id: target.id});
        startTransition(async () => {
            ...
            if (error) handleError(error);
                ...
                setTargetBtn(null);
                successToast('Vote added!', 'Success');
            }
        })
    }    
    ...
    const handleAcceptAnswer = () => {
        ...
        startTransition(async () => {
            ...
            if (error) handleError(error);
            else successToast('Answer accepted!', 'Success');
        })
    }
    ...
    return (
        <div className='shrink-0 flex flex-col gap-3 items-center justify-start mt-4'>
            <Button isIconOnly
                    variant='light'
                    isLoading={pending && targetBtn?.id === target.id && targetBtn.type === 'up'}
                    isDisabled={!canVote}
            ...
            <Button isIconOnly
                    variant='light'
                    isLoading={pending && targetBtn?.id === target.id && targetBtn.type === 'down'}
                    isDisabled={!canVote}
                    onPress={handleAddVote(-1)}
            >
